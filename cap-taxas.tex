...
Normalmente o gerador infinitesimal é usado no estudo de processos
estocásticos para garantir a existência do processo estudado. Se
inicia com um gerador e argumenta-se, usando o teorema de 
Hille-Yosida, que existe um processo com esse gerador.

Nós estamos indo na direção oposta. Já temos uma construção do nosso
processo, e queremos saber qual a ``cara'' do seu
gerador. 

Aqui vamos nos limitar ao caso homogêneo, isso é, $\lambda_x = 1$ para
todo $x \in \Nz$. Vamos ainda tratar apenas o caso $c = 0$.

O primeiro passo vai ser definir a classe de funções onde iremos
definir o gerador. Essa classe tem que ser pequena o suficiente para
que o gerador esteja bem definido, mas rica o suficiente para que ele
caracterize o semigrupo. A classe adotada será:

\begin{equation}
  \label{eq:classe-gerador}
  \DDD = \left\{ f \in \CC: \sum_{x\in \Nz} |f(x)-f(\infty)| < \infty,
    \:
    \sum_{x\in \Nz} f(x)-f(\infty) = 0, \:
...
Agora vamos mostrar que esse limite de fato existe, e calcular o seu valor.
...
\begin{proof}
  ...
  $f(y) < f(\infty) + \epsilon$ para $y > m$. Ainda com base na
  Proposições \ref{prop:taxa-x-y} concluímos que cada termo da série
  acima converge para zero quanto $t \searrow 0$. Portanto:
  \begin{align*}
    \limsup_{t \searrow 0}
    \frac{\Psi_t f(x) - f(x)}{t}
    &= \limsup_{t \searrow 0}
    \sum_{y > m} \left[ f(y) - f(x) \right] \frac{p_{x y}(t)}{t}\\
    &\leq \limsup_{t \searrow 0} \left[ f(\infty) - f(x) + \epsilon \right]
    \sum_{y > m} \frac{p_{x y}(t)}{t}\\
    &=\limsup_{t \searrow 0} \left[ f(\infty) - f(x) + \epsilon \right]
...
\begin{proof}
  ...
  -\bar{h}(x)/\gamma_x$, limite que existe pela definição de $\DDD$.

  Por toda essa demonstração, vamos supor que $\gamma_1 \geq g_x$ para
  todo $x \in \Nz$, $x > 2$. Não perdemos generalidade
  ao fazer essa suposição porque basta trocar $1$  no nosso
  argumento pelo estado que tem $\gamma$ máximo. Tal estado
  existe porque $\gamma_x \to 0$ quando $x \to \infty$.

  Outra notação que iremos usar bastante será:
  \begin{displaymath}
    \Gamma^{<x>}(t) = \sum_{y\in\Nz\setminus\{x\}} \sum_{i =
      1}^{N^y(t)} \gamma_y T^y_i ,
  \end{displaymath}
  essa é a função $\Gamma$, do processo iniciado no $\infty$ com
  $c=0$, mas excluímos o estado $x$ da somatória.  

  Para um $\epsilon > 0$ fixado arbitráriamente, vamos mostrar que:
  \begin{align}
    ...
    &\geq K - \epsilon,
  \end{align}
  de onde concluíremos \eqref{eq:gerador-inf} imediatamente.
  ...
  \begin{align*}
    ...
    &= 
    \sum_{x \in \Nz} \bar{h}(x) P\left( X^\infty(t) = x \text{ pela
        primeira vez}\right)\\
    &+ \sum_{x \in \Nz} \bar{h}(x) P\left( X^\infty(t) = x \text{ não pela
        primeira vez}\right)
  \end{align*}

  Tomando $f = |\bar{h}|$ no Lema \ref{lema:gerador-erro} vemos que o
  segundo termo, dividido por $t$ converge para zero quando $t
  \searrow 0$, assim podemos nos concentrar apenas nas primeiras
  visitas. Usando o fato de que $\sum_{x\in\Nz}\bar{h}(x) = 0$,
  podemos escrever o primeiro termo da expressão acima como:
  \begin{displaymath}
    \sum_{x \in \Nz} \bar{h}(x) \left[
      P\left( X^\infty(t) = x \text{ pela primeira vez} \right) - 1 
    \right]
  \end{displaymath}

  Note que $X^\infty(t) = x$ pela primeira vez se e somente se
  $\Gamma^{<x>}(\sigma^x_1) \leq t < \Gamma^{<x>}(\sigma^x_1) +
  \gamma_x T^x_1$. Assim se denotarmos por $f_x$ a densidade de
  $\Gamma^{<x>}(\sigma^x_1)$ e por $F_x$ a sua função de distribuição,
  teremos que:
  \begin{align*}
    \sum_{x \in \Nz} \bar{h}(x) \left[
      P\left( X^\infty(t) = x \text{ pela primeira vez} \right) - 1 
    \right]
    &= - \sum_{x \in \Nz} \bar{h}(x) \left[
      \int_0^t f_x (s) \left(1 - e^{-\frac{t-s}{\gamma_x}} \right) ds
    \right]\\
    &= - \sum_{x \in \Nz} \frac{\bar{h}(x)}{\gamma_x} \left[
      \int_0^t F_x (s) e^{-\frac{t-s}{\gamma_x}} ds
    \right],
  \end{align*}
  onde a última igualdade veio de uma integração por partes.

  Como $F_x(0) = 0$, temos que cada termo da série acima, dividido por
  $t$ converge para zero quando $t \searrow 0$. Assim podemos nos
  preocupar apenas com a cauda da série. Dessa forma tome um $m$ tal
  que $-\frac{\bar{h}(x)}{\gamma_x} < K + \epsilon$ sempre que $x >
  m$.

  Como $\gamma_1 \geq \gamma_x$, então $\Gamma^{<x>}$ domina
  estocasticamente $\Gamma^{1}$. Dessa forma $F_x(t) \leq
  F_1(t)$.
  ...
  \begin{align*}
    ...
    &\leq \left( K + \epsilon \right) \limsup_{t \searrow
      0}\frac{1}{t} \int_0^t F_1(s) \sum_{x >
      m}e^{-\frac{t-s}{\gamma_x}} ds\\
    &= \left( K + \epsilon \right) \limsup_{t \searrow
      0}\frac{1}{t} \int_0^t F_1(s) \left[
      1 + \sum_{x > 1} e^{-\frac{t-s}{\gamma_x}}
    \right] ds\\
    &- \left( K + \epsilon \right) \limsup_{t \searrow
      0}\frac{1}{t} \int_0^t F_1(s) \left[
      1 + \sum_{x=2}^m e^{-\frac{t-s}{\gamma_x}}
    \right] ds\\
  \end{align*}

  O segundo termo dessa soma vale zero já que $F_1(s) \to 0$ quando $s
  \searrow 0$ e a soma lá é finita. Resta calcular o primeiro
  termo. Para isso defina:
  \begin{align*}
    \Phi(t) &:= \int_0^t F_1(s) \left[
      1 + \sum_{x > 1} e^{-\frac{t-s}{\gamma_x}}
    \right] ds.
  \end{align*}
  ...
  fixado, usando integração por partes, podemos calcular a tranformada
  de Laplace da primeira por:
  \begin{align*}
    \int_0^{\infty} e^{-\beta t} F_1(t) dt 
    &= \frac{1}{\beta} \int_0^\infty e^{-\beta t}  f_1(t) dt\\
    &= \frac{1}{\beta}
    \E \left[ \exp \{-\beta \Gamma^{<1>} (\sigma_1^1)\}  \right]\\
    &= \frac{1}{\beta}
    \left( 1 + \sum_{x > 1}
  ...
  \begin{align*}
    \int_0^{\infty} e^{-\beta t} \left[
      1 + \sum_{x > 1} e^{-\frac{t}{\gamma_x}}
    \right]  dt 
    &= \frac{1}{\beta} + \sum_{x > 1} \int_0^\infty \exp \left\{
      - \left(\beta + \frac{1}{\gamma_x} \right) t \right\} dt\\
    &= \frac{1}{\beta} + \sum_{x > 1} \frac{\gamma_x}{1 +
      \beta\gamma_x}\\
    &= \frac{1}{\beta} \left(
      1 + \sum_{x > 1} \frac{\beta\gamma_x}{1 +
      \beta\gamma_x}
    \right)
  \end{align*}
  ...
  Agora indentificamos essa transformada de Laplace com a da
  identidade, isso é:
  \begin{displaymath}
  ...
  $\Gamma^{<x>}$ domina $\Gamma^{<1>}$, usaremos que $\Gamma$ domina
  $\Gamma^{<x>}$. Também teremos que colocar $x = 1$ na soma que
  define $\Phi$.
\end{proof}
...
\begin{lema}
  ...
  \begin{equation}
    ...
    P\left( X^\infty(t) = x \text{ não pela
        primeira vez}\right) = 0
  \end{equation}
\end{lema}
\begin{proof}
  Novamente vamos supor, sem perda de generalidade, que $\gamma_1 \leq
  ...
  \begin{align*}
    ...
    &= P \left( -\frac{\Gamma^{<1>}(\sigma^1_2)}{t} \geq - 1 \right)\\
    &\leq \mathrm{cte }\:
    \E \left[  -\frac{\Gamma^{<1>}(\sigma^1_2)}{t}  \right]\\
  ...
  O evento $ \{X^\infty(t) = x \text{ não pela primeira vez}\}$ está
  contido no evento $\{ \Gamma^{<x>} (\sigma^x_2) \leq t, \gamma_x
  T^x_1 \leq t\}$. Assim usando a estimativa acima e o fato que
  $\Gamma^{<x>} (\sigma^x_2)$ é independente de $T_1^x$, obtemos que:
  \begin{align*}
  ...
  Vamos mostrar que cada um desses termos converge à zero quando $t
  \searrow 0$.
  ...
  uma constante. Se denotarmos pot $\#A_t$ a cardinalidade de $A_t$,
  poderemos escrever:
  \begin{align*}
    \left( \sum_{y \in \Nz} \frac{\gamma_y}{t + \gamma_y} \right)^{-2}
    \sum_{x \in A_t} f(x) \frac{1 - e^{-t/\gamma_x}}{t}
  ...
  \gamma_x$. Juntanto isso ao fato de que $1 - e^{-t/\gamma_x} \leq 1$
  podemos escrever:
  \begin{align}
  ...
  Agora vamos separar em 2 casos. O primeiro ocorre quando $\sum_{x
    \not\in A_t} \frac{\gamma_x}{t}$ vai para o infinito quanto $t
  \searrow 0$.
  ...
  O segundo caso ocorre quando $\sum_{x
    \not\in A_t} \frac{\gamma_x}{t}$. Nesse caso é fácil ver que o
  denominador de \eqref{eq:erro-gerador-quase} diverge, assim a
  expressão converge para zero quanto $t \searrow 0$.  
\end{proof}
...
\begin{proof}
  ...
  \epsilon$ para $x > n_0$. Fixe um tal $n_0$. Defina $g(x) = f(x)$
  ...
  f(x)| < \epsilon$. Assim $\Vert f - g \Vert < \epsilon$.
