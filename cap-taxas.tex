...
\begin{proposicao}
  ...
  \begin{align*}
    &p_{xy}(t) \geq 0\\
    &\sum_{y \in \Nzb} p_{xy}(t) = 1\\
    &p_{xy}(t+s) = \sum_{z\in\Nzb}p_{xz}(t)p_{zy}(s)\\
    & p_{xy}(\bullet) \textrm{ é uma função Lebesgue mensurável.}
  \end{align*}
\end{proposicao}
\begin{proof}
...
\begin{proof}
  Esse é o Teorema 1 da primeira seção da Parte II (pg. 120) de
  \cite{chung:67}.
\end{proof}
...
\begin{proof}
  ...
  Portanto $\E [\theta(t)] > 0$. Usando o teorema de Fubbini obtemos
  que:
  \begin{displaymath}
  ...
  Agora considere $u := \liminf_{t \searrow 0}p_{\infty \infty} (t)$,
  e tome uma sequência $(t_n)$, onde $t_n \to 0$ e $p_{\infty \infty}
  (t_n) \to u$ quanto $n \to \infty$. Agora usando a Proposição
  \ref{prop:transicao-continua}, temos que:
  \begin{displaymath}
  ...
  Dessa forma, já que $p_{\infty \infty}(s) > 0$, concluímos que $u =
  1$. De forma que:
  \begin{displaymath}
    \liminf_{t \searrow 0} p_{\infty\infty}(t) = 1 \geq \limsup_{t
      \searrow 0} p_{\infty\infty}(t),
  \end{displaymath}
  de onde concluímos que $p_{\infty\infty}(t) \to 1$ quando $t \searrow 0$.
\end{proof}
