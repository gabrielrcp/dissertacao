...
e argumenta-se, usando o teorema de Hille-Yosida, que existe um
processo com essa expressão como gerador.

Nós estamos indo na direção oposta. Já temos uma construção do nosso
processo, e queremos saber qual a ``cara'' do seu gerador.

Vamos nos limitar ao caso homogêneo, isso é, $\lambda_x = 1$ para
...
\begin{proof}
  ...
  Por toda essa demonstração, vamos supor que $\gamma_1 \geq \gamma_x$
  para todo $x \in \Nz$. Não perdemos generalidade ao fazer
  essa suposição porque basta trocar $1$ no nosso argumento pelo
  estado que tem $\gamma$ máximo. Tal estado existe porque $\gamma_x
  ...
  \begin{align*}
    ...
    \right]
    &= \sum_{x \in \Nz} \bar{h}(x) \left[
      \int_0^t f_x (s) e^{-\frac{t-s}{\gamma_x}} ds - 1
    \right]\\
    &= - \sum_{x \in \Nz} \bar{h}(x) \left[
  ...
  0$. Assim podemos nos preocupar apenas com a cauda da série. Dessa
  forma tome um $m$ tal que $-\frac{\bar{h}(x)}{\gamma_x} < K +
...
\begin{proof}
  ...
  \frac{\gamma_x}{t}$ é limitado. Nesse caso é fácil ver que o
