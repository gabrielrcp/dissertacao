...
\begin{proof}
  ...
  Tome uma sequência abitrária $(t_n)$, onde $t_n \searrow 0$ e
...
\begin{proof}
  ...
  probabilidade de sucesso dada por $e^{-\frac{h}{\gamma_y}}$. Observe
  que $X_t^y = y$ para todo $t \in [\Gamma^y(\sigma_M^x-),
  \Gamma^y(\sigma_M^x-) + T_M^y)$ e, como $T_M^y > h$, existe um
  múltiplo de $h$ nesse intervalo. Assim se $n h$ for esse múltiplo,
  vai valer que $Y_n^{y, h} = y$. Dessa maneira a esperança de tempo
  de retorno à um estado em $(Y_n)$ é finita.
...
somemente o processo K iniciado no $\infty$, visto que o conjunto dos
