...
\begin{proof}
  ...
  Se $\displaystyle \limsup_{x \to \infty} \frac{\log \gamma_x}{\log
    x} = -1$, então \eqref{eq:log-haus-sup} é óbvia. Dessa maneira
  vamos tomar $\delta > 0$ tal que:
  \begin{displaymath}
    \limsup_{x \to \infty} \frac{\log \gamma_x}{\log x} = -(1
    + \delta).
  \end{displaymath}
  ...
  Como o $\epsilon$ pode ser tomado arbitrariamente pequeno,
  concluímos \eqref{eq:log-haus-sup}.
...
\end{corolario}
\begin{proof}
  Esse corolário é uma consequencia direta do Corolário
  \ref{cor:log-haus}.
\end{proof}
