...
\begin{proof}
  ...
  Assim se $\nu$ for a distribuição de $\sum_{x \geq 2} \sum_{i =
    1}^{N^x(s)} \gamma_x T_i^x$, temos que:
  \begin{align*}
  ...
  que é a lei de $\sigma^x_i$, temos que:
  \begin{align*}
...
\begin{proof}
  ...
  Fixemos uma realização do processo e fixe $T > 0$ e $\epsilon > 0$
  ...
  Tomemos um $m \in \Nz$ tal que o $\diam\{x \in \Nzb: x > m \} =
...
\begin{proof}
  ...
  T_0$, temos que o processo iniciado em $y$ será uma cópia do
  processo iniciado no $\infty$, e como antes disso temos
  $\lambda^y_n(t) = t$ e $X^y(t) = y = X^{y, (n)}(t)$, então concluímos
...
Markoviano. Para demonstrar que essaa propriedade é mantida, vamos
...
\begin{proof}
  ...
  \qc se e somente se as a soma de suas médias convergirem.
...
\begin{proof}
  ...
  Primeiro mostremos que cada ``termo'' de \eqref{eq:convergencia-semigrupo}
