...
% Poderíamos ter adotado uma abordagem um pouco diferente, e mostrado
% que a sequência de trajetórias $(X^{(n)})$ formam \qc uma sequência de
% Cauchy numa métrica equivalente à de Skorohod e que é completa.

% Se mostrassemos isso, então concluiríamos que existe um processo
% $\tilde{X}$ os processos truncados convergem para $\tilde{X}$ \qc na
% métrica de Skorohod.

% Depois disso, se mostrarmos a convergência  poderíamos mostrar que, para todo $t > 0$ fixo,
% $X^{(n)}(t) \xrightarrow{$
%  dessa
% forma existe 
