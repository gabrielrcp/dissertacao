...
\begin{proof}
  ...
  Denote por $\Psi$ e $\Psi^n$ os semigrupos de transição  de $X$ e
  $X^{(n)}$ rescpectivamente, isso é, para $t > 0$ e $f: \Nzb \to \R$:
  \begin{gather*}
    \Psi_t f (x) = \E \left[ X^x(t) \right]\\
    \Psi^n_t f (x) = \E \left[ X^{x, (n)}(t) \right]
  \end{gather*}

  Agora fixado um $m \geq 1$, $t_1 < t_2 < \ldots < t_{m+1}$ e $f_1,
  \ldots, f_{m+1} : \Nzb \to \R$ funções contínuas.
  ...
  \begin{displaymath}
    |\epsilon_n| \leq \sup_{y \in \Nzb} \left\lvert
